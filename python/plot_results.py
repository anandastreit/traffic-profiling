...
Generates two figures saved to python/figures/:
  1. modeb_profiles.png  — time-of-day loading profiles for each component
                           Python (tol=1e-6) overlaid on MATLAB reference
...
WEEK_MAP = {"1": "week01", "2": "week02", "3": "week03", "4": "week04", "5": "week05"}
...
def plot_modeb_profiles(df):
    ...
    # Reorder Python columns to match MATLAB component order
    py_b_aligned = py_b[:, col_ind]

    minutes = np.arange(1440)
    x_ticks = np.arange(0, 1441, 120)          # every 2 hours
    x_labels = [minutes_to_time(m) for m in x_ticks]

    fig, axes = plt.subplots(N_COMP, 1, figsize=(12, 10), sharex=True)
    fig.suptitle("modeB — Time-of-day profiles (all_days model)\n"
                 "Python tol=1e-6 vs MATLAB reference", fontsize=12)
    ...
    for i, ax in enumerate(axes):
        ...
        # normalise to [0,1] for visual comparison (scale ambiguity is expected)
        def norm01(v):
            lo, hi = v.min(), v.max()
        ...
        ax.plot(minutes, norm01(py_col),  color=colors[i], lw=1.2,
                linestyle="--", alpha=0.75, label=f"Python comp {col_ind[i]+1}")
...
def plot_tcc_heatmap(df):
    # Build matrix: rows = weeks (1-5) + reference, cols = components 1-5
    week_labels = [f"Week {w}" for w in range(1, 6)]
    ref_label   = ["Reference"]
    row_labels  = week_labels + ref_label

    tcc_mean_b = []  # mean TCC for modeB per row
    tcc_vals_b = []  # per-component TCC for modeB
    ...
    ax.set_title("TCC: Python vs MATLAB — modeB (time profiles)\n"
